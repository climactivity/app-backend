...
import Toast from "$lib/components/Toast/toast.svelte";
    ...
    const save = async () => {
        saving = true
        const res = await fetch(`${baseUrl}${saveTarget}${data._id ? "/" + data._id :""}`, {
            credentials: "include",
...
<div class="bg-white rounded-b-md shadow-md fixed ">
    <nav class="grid grid-cols-3  ">
        ...
        <dev  class="flex flex-row col-span-1 col-start-3 place-content-end mr-4">
            ...
            <label class="text-lg my-auto">Veröffentlichen <input type="checkbox" bind:checked={data.published}/></label>

            <Toast let:triggerToast>
                <button class=" button cta ml-2 my-auto " class:saving  on:click={async() => {
                    let ok = await save();
